...
public class FrontEndController {
    ActorkeysByFilmkey actorByFilm = new ActorkeysByFilmkey();
    ...
    @GetMapping("/testactorsbyfilm")
    public String testactorbyfilm(Model model) {
        model.addAttribute("getstring", new Getstring("actor name here"));
        return "testactorsbyfilm";
    }

    @PostMapping("/testactorsbyfilm")
    public String submitTestActorsByFilm(@ModelAttribute Getstring getstring, Model model) {
        String result = "You typed film key:" + getstring.getContent();
        List <String> actorkeys = actorService.findActorKeysBy(getstring.getContent());
        for (String s: actorkeys) {
            result += "- " + s;
        }
        Getstring svresult = new Getstring(result);
