...
public class CalculateSeparation {
    ...
    */
    ...
    public Separationlist generateNextSeparationlist(Separationlist aActorlist) {
    ...
    public int checkLevelOfSeparation(Resultstrings aResultstrings) throws Exception {
        ...
        //check level 1
        generateSeparationlistForActor(true, 1);
        int result = checkNext(true, 1, 0, aResultstrings);
        if (result >0 ) return result;

        // check level 2
        generateSeparationlistForActor(false, 1);
        result = checkNext(false, 1, 1, aResultstrings);
        if (result >0 ) return result;

        // check level 3
        generateSeparationlistForActor(false, 2);
        result = checkNext(true, 1, 2, aResultstrings);
        if (result >0 ) return result;

        // check level 4
        generateSeparationlistForActor(true, 2);
     ...
     *
     * ******************************************************************************************/
    void generateSeparationlistForActor(boolean aFirstActor, int aLevel)  throws Exception {
        ...
        lists.set(aLevel, generateNextSeparationlist(lists.get(aLevel - 1)));
