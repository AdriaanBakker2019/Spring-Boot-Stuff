...
public interface FilmRepository extends JpaRepository<Film, Long> {
    ...
    @Query(value = "select tconst, primaryTitle primary_title from titlebasics where tconst =  ?1 ", nativeQuery = true)
    ...
    @Query(value = "select tconst from titleprincipals where nconst = ?1" +
                   ...
                   " and titleType in ('short','movie','tvShort', 'tvMovie'))",
                   nativeQuery = true)
    public List<String>  findTitlesInCommon(String ncode1, String ncode2);
}